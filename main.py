...
def load_options():
    opts = {
        ...
        "leadtype": ["Inbound", "AI Scan", "Andet"], "titles": ["CEO", "Ejer", "Manager"],
...
def lead_popup(idx):
    ...
    with t2:
        ...
        c1, c2 = st.columns(2)
        with c1:
            upd['Region'] = st.selectbox(L['f_reg'], opts['regions'], index=opts['regions'].index(row['Region']) if row['Region'] in opts['regions'] else 0)
            upd['Town'] = st.selectbox(L['f_town'], opts['towns'], index=opts['towns'].index(row['Town']) if row['Town'] in opts['towns'] else 0)
            upd['Område Type'] = st.selectbox(L['f_type'], opts['area_types'], index=opts['area_types'].index(row['Område Type']) if row['Område Type'] in opts['area_types'] else 0)
            for f, lab in [('Area', 'f_area'), ('Address', 'f_addr'), ('Postal Code', 'f_zip')]:
                upd[f] = c1.text_input(L[lab], value=row.get(f,''), key=f"f2_{f}_{idx}")
        with c2:
            upd['Brancher'] = st.selectbox(L['f_br'], opts['brancher'], index=opts['brancher'].index(row['Brancher']) if row['Brancher'] in opts['brancher'] else 0)
            ...
            upd['Underbrancher'] = ", ".join(st.multiselect(L['f_ubr'], sub_l, default=[x for x in curr_ubr if x in sub_l]))
            curr_l = [x.strip() for x in str(row.get('Languages')).split(',')] if row.get('Languages') else []
            upd['Languages'] = ", ".join(st.multiselect(L['f_lang'], opts['sprog'], default=[x for x in curr_l if x in opts['sprog']]))
    ...
    with t3:
        ...
        with c2:
            ...
            upd['Leadtype'] = st.selectbox(L['f_src'], opts['leadtype'], index=opts['leadtype'].index(row.get('Leadtype')) if row.get('Leadtype') in opts['leadtype'] else 0)
...
with st.sidebar:
    ...
    if st.session_state.user_role == "admin":
        with st.expander(L['sidebar_admin']):
            ...
            if st.button("Opret Agent"):
                ...
                st.success("OK")
            st.divider()
            cat_list = ["agents", "status", "membership", "advertising", "leadtype", "titles", "sprog", "regions", "towns", "brancher", "underbrancher", "area_types"]
            ...
            if custom_opts[cat_ed]:
                v_del = st.selectbox("Slet:", ["Vælg..."] + custom_opts[cat_ed])
    ...
    if st.button(L['btn_create'], type="primary", use_container_width=True):
        nums = pd.to_numeric(st.session_state.df_leads['Client ID'], errors='coerce').dropna()
        nr = {c: "" for c in MASTER_COLS}; nr['Date created'] = date.today().strftime('%d/%m/%Y'); nr['Client ID'] = int(nums.max()+1) if not nums.empty else 1001
        st.session_state.df_leads = pd.concat([st.session_state.df_leads, pd.DataFrame([nr])], ignore_index=True)
        save_db(st.session_state.df_leads); st.rerun()
