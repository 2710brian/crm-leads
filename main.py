...
# --- 2. DATABASE MOTOR (POOL MANAGEMENT) ---
@st.cache_resource
def get_engine():
...
# --- 3. SPROG-DATABASE (ALLE NØGLER SIKRET 100%) ---
TRANSLATIONS = {
    "🇩🇰 Dansk": {
    ...
    "🇬🇧 English": {
        ...
        "f_mem": "Membership", "f_adv": "Ads", "f_agent": "Agent", "f_src": "Source", "f_created": "Created", 
    ...
    "🇪🇸 Español": {
        ...
        "f_desc": "Descripción", "f_qr": "URL QR", "f_notes": "Notas", "field_logo": "Logo", "field_docs": "Docs", "field_gal": "Galería", "f_type": "Tipo Zona"
    }
}
...
COSTA_DEL_SOL_AREAS = ["Costa del Sol Vest", "Costa del Sol Midt", "Costa del Sol Øst", "Costa de la Luz", "Costa Tropical", "Costa de Almería", "Costa Blanca", "Costa Brava", "Inland", "Islands"]
...
def load_options():
    opts = {
        ...
        "kilde": ["Inbound", "AI Scan", "Andet"], "titles": ["CEO", "Ejer", "Manager", "Marketingchef"],
        "sprog": ["Dansk", "Engelsk", "Spansk", "Svensk", "Norsk", "Tysk"],
        "area_types": AREA_TYPES, "regions": REGIONS, "areas": COSTA_DEL_SOL_AREAS, "towns": TOWNS,
        "brancher": sorted(list(INDUSTRIES.keys())), "underbrancher": [u for sub in INDUSTRIES.values() for u in sub]
    }
    if db_engine:
        try:
...
# --- 7. POPUP KORT (DE 5 FANER - KOMPLET) ---
...
def lead_popup(idx):
    ...
    with t2:
        ...
        with c1:
            ...
            upd['Area'] = c1.selectbox(L['f_area'], opts['areas'], index=opts['areas'].index(row['Area']) if row['Area'] in opts['areas'] else 0)
            upd['Town'] = c1.selectbox(L['f_town'], opts['towns'], index=opts['towns'].index(row['Town']) if row['Town'] in opts['towns'] else 0)
            for f, lab in [('Address', 'f_addr'), ('Postal Code', 'f_zip')]:
                upd[f] = c1.text_input(L[lab], value=row.get(f,''), key=f"f2_{f}_{idx}")
        with c2:
            upd['Brancher'] = c2.selectbox(L['f_br'], opts['brancher'], index=opts['brancher'].index(row['Brancher']) if row['Brancher'] in opts['brancher'] else 0)
            sub_l = INDUSTRIES.get(upd['Brancher'], opts['underbrancher'])
    ...
    with t5:
        ...
        gal_up = st.file_uploader("Vælg galleri filer", accept_multiple_files=True, key=f"ga_{idx}")
        if gal_up:
            gal_list = []
...
with st.sidebar:
    ...
    # ADMIN KONTROL (ALLE DROPDOWNS OG AGENTER)
    if st.session_state.user_role == "admin":
        with st.expander(L['sidebar_admin']):
            # BRUGERE
            st.markdown("##### 👤 Agenter")
            nu, np = st.text_input("Username"), st.text_input("Pass", type="password")
            if st.button("Add Agent"):
                with db_engine.begin() as conn: conn.execute(text("INSERT INTO users VALUES (:u,:p,'agent')"), {"u":nu,"p":np})
                st.rerun()
            st.divider()
            # LISTER
            cat_list = ["agents", "status", "membership", "advertising", "kilde", "titles", "sprog", "regions", "towns", "brancher", "underbrancher", "area_types"]
