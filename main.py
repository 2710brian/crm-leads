...
# --- 2. KOMPLET SPROG-DATABASE (FIXET FOR ALLE KEYERRORS) ---
TRANSLATIONS = {
    "🇩🇰 Dansk": {
        "login_title": "CRM Master Login", "login_btn": "LOG IND", "logout": "🚪 Log ud",
        "search": "🔍 Søg i alt data...", "total_leads": "Viste leads: {n}", "click_info": "💡 Klik på ✏️ for at åbne kortet.",
    ...
    "🇬🇧 English": {
        "login_title": "CRM Master Login", "login_btn": "LOG IN", "logout": "🚪 Log out",
        "search": "🔍 Search data...", "total_leads": "Leads shown: {n}", "click_info": "💡 Click ✏️ to open.",
        "sidebar_scan": "📸 AI Card Scanner", "sidebar_filter": "🎯 Campaign Filters", "sidebar_admin": "🛠️ Admin Control",
        "sidebar_user": "👤 User Management", "sidebar_export": "📤 Export", "btn_create": "➕ CREATE MANUAL",
        "btn_save": "💾 SAVE ALL CHANGES", "btn_delete": "🗑️ DELETE", "btn_bulk_delete": "🗑️ DELETE SELECTED",
        "tab1": "📞 Contact & Social", "tab2": "🌍 Geography", "tab3": "⚙️ Sales", "tab4": "📝 Desc", "tab5": "📁 Media",
        "f_id": "Client ID", "f_name": "Company Name", "f_cif": "CIF / VAT", "f_person": "Contact", 
        ...
        "f_br": "Industries", "f_ubr": "Sub-industries", "f_lang": "Languages", "f_work": "Work Hours", "f_st": "Status", 
        ...
        "f_qr": "QR URL", "f_notes": "Internal Notes", "f_logo": "Logo", "f_docs": "Docs", "f_gal": "Gallery"
    },
    "🇪🇸 Español": {
        "login_title": "Entrar CRM", "login_btn": "ENTRAR", "logout": "🚪 Salir",
        "search": "🔍 Buscar...", "total_leads": "Leads: {n}", "click_info": "💡 Haga clic en ✏️ para abrir.",
        ...
        "tab1": "📞 Contacto", "tab2": "🌍 Geografía", "tab3": "⚙️ Ventas", "tab4": "📝 Descripción", "tab5": "📁 Medios",
        ...
        "f_mem": "Membresía", "f_adv": "Perfil Anuncio", "f_agent": "Agente", "f_src": "Origen",
...
DISPLAY_COLS = ['✏️', 'Client ID', 'Date created', 'Company Name', 'Brancher', 'Town', 'Status on lead', 'Agent']

def load_options():
    opts = {
        "agents": ["Brian", "Olga"], "status": ["Ny", "Dialog", "Vundet", "Tabt"], 
        "membership": ["BASIC", "VIP", "Premium", "Gold"], 
        "ad_profile": ["Standard", "Proff.", "SoMe"], "kilde": ["Inbound", "AI Scan"], "titles": ["CEO", "Ejer"]
    }
...
def lead_popup(idx):
    ...
    with t2:
        ...
        with c1:
            ...
            upd['Area'] = c1.selectbox(L['f_area'], ["Vest", "Midt", "Øst", "Inland"], index=0)
            upd['Address'] = c1.text_input(L['f_addr'], value=row.get('Address'))
        with c2:
            ...
            upd['Languages'] = c2.text_input(L['f_lang'], value=row.get('Languages'))
    with t3:
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            ...
            for f, lab in [('Date created','f_created'), ('Date for follow up','f_follow')]:
...
# --- 8. SIDEBAR ---
with st.sidebar:
    st.session_state.lang = st.selectbox("🌐 Choose Language", list(TRANSLATIONS.keys()))
    ...
    if st.session_state.user_role == "admin":
        with st.expander(L['sidebar_admin']):
            ...
            if st.button("Create"):
                ...
                st.success("OK")
            st.divider()
            cat = st.selectbox("Edit List:", ["agents", "status", "membership", "ad_profile"])
...
if 'sel_rows' in st.session_state and len(st.session_state.sel_rows) > 0 and st.session_state.user_role == "admin":
    if c_btn1.button(L['btn_bulk_delete'], type="secondary"):
...
# BULK SELECTOR
st.info("💡 Use table below for bulk delete/download:")
