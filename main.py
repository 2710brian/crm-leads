...
TRANSLATIONS = {
    ...
    "🇬🇧 English": {
        ...
        "f_follow": "Follow up", "f_last": "Contact date", "f_pitch": "Pitch", "f_desc": "Long Desc", 
...
def lead_popup(idx):
    ...
    upd = {}
    with t1:
        c1, c2 = st.columns(2); upd['Client ID'] = c1.text_input(L['f_id'], value=row.get('Client ID'))
        upd['CIF Number VAT'] = c2.text_input(L['f_cif'], value=row.get('CIF Number VAT'))
        st.divider()
        ct1, ct2 = st.columns(2); upd['Company Name'] = ct1.text_input(L['f_name'], value=row.get('Company Name'))
...
with st.sidebar:
    ...
    if st.session_state.user_role == "admin":
        with st.expander(L['sidebar_admin']):
            ...
            st.divider()
            # LISTER
            cat = st.selectbox("Edit:", ["agents", "status", "membership", "ad_profile", "kilde", "titles", "sprog"])
            v_new = st.text_input("Value:")
    ...
    f_ag = st.multiselect(L['f_agent'], opts['agents'])
