...
TRANSLATIONS = {
    "🇩🇰 Dansk": {
        "title": "Business CRM Master AI", "login_title": "MGM Login", "login_btn": "LOG IND", "logout": "🚪 Log ud",
    ...
    "🇬🇧 English": {
        ...
        "search": "🔍 Search...", "total_leads": "Leads: {n}", "click_info": "💡 Click row to open card.",
...
SPANISH_REGIONS = {
    ...
    "Øer": ["Mallorca", "Ibiza", "Menorca", "Formentera", "Tenerife", "Gran Canaria", "Lanzarote", "Canary Islands", "Balearic Islands"]
}
...
def load_options():
    defaults = {
        ...
        "towns": ["Marbella", "Málaga", "Estepona", "Fuengirola"]
    }
    if db_engine:
        try:
            ...
            for k in defaults.keys():
                ...
                if stored: defaults[k] = sorted(list(set(defaults[k] + stored)))
        except: pass
    return defaults
...
# --- 8. POPUP KLIENT KORT ---
@st.dialog("🎯 CRM Detail View", width="large")
def lead_popup(idx):
    ...
    with t1:
        st.markdown("##### 🏢 Virksomhed")
        c1, c2 = st.columns(2)
        upd['Company Name'] = c1.text_input(L['f_name'], value=row.get('Company Name'))
    ...
    with t2:
        ...
        with c1:
            ...
            reg_main = list(SPANISH_REGIONS.keys())
            sel_reg_main = c1.selectbox("Region Gruppe", reg_main)
            upd['Region'] = c1.selectbox(L['f_reg'], SPANISH_REGIONS[sel_reg_main])
            upd['Area'] = c1.text_input(L['f_area'], value=row.get('Area'))
        ...
        with c2:
            ...
            upd['Postal Code'] = c2.text_input(L['f_zip'], value=row.get('Postal Code'))
    ...
    with t3:
        ...
        with c1:
            ...
            upd['Advertising'] = st.selectbox(L['f_adv'], opts['ad_profile'], index=0)
        with c2:
            ...
            upd['Kontakt dato'] = st.date_input("Last Contact", value=date.today()).strftime('%d/%m/%Y')
...
with st.sidebar:
    ...
    if st.session_state.user_role == "admin":
        with st.expander("👤 Administrer Agenter"):
        ...
        with st.expander("🛠️ Administrer Lister"):
            cat = st.selectbox("Vælg Liste:", ["agents", "status", "membership", "ad_profile", "towns", "titles"])
            v_add = st.text_input("Tilføj værdi:")
            if st.button("Tilføj"):
                with db_engine.begin() as conn: conn.execute(text("INSERT INTO crm_configs (type, value) VALUES (:t,:v)"), {"t":cat,"v":v_add})
                st.rerun()
...
# VIKTIGT: Vi viser kun DISPLAY_COLS + flueben for at det åbner kortet
sel_df = df_v[['Client ID', 'Date created', 'Company Name', 'Brancher', 'Status on lead']]
sel = st.dataframe(sel_df, use_container_width=True, selection_mode="single-row", on_select="rerun", height=600)
