...
TRANSLATIONS = {
    "🇩🇰 Dansk": {
        ...
        "f_id": "Klient ID", "f_name": "Virksomhed", "f_cif": "CIF / VAT", "f_person": "Kontaktperson", 
        ...
        "f_area": "Område", "f_town": "By", "f_addr": "Adresse", "f_zip": "Postnr", "f_loc": "Maps Link", 
        "f_br": "Branche", "f_ubr": "Underbranche", "f_lang": "Sprog", "f_work": "Timer", "f_st": "Status", 
        "f_mem": "Medlem", "f_adv": "Annonce", "f_agent": "Agent", "f_src": "Kilde", "f_created": "Oprettet", 
        "f_follow": "Opfølgning", "f_last": "Sidste kontakt", "f_pitch": "Kort Pitch", "f_desc": "Tekst", 
        "f_qr": "QR Tracking URL", "f_notes": "Noter", "field_logo": "Logo", "field_docs": "Dokumenter", "field_gal": "Galleri", "f_type": "Område Type"
    },
    "🇬🇧 English": {
        "title": "Business CRM AI Pro", "login_title": "CRM Login", "login_btn": "LOG IN", "logout": "🚪 Log out",
        "search": "🔍 Search...", "total_leads": "Leads: {n}", "click_info": "💡 Click row on the left.",
...
# --- 4. DATA LISTER (GEOGRAFI & BRANCHER) ---
GEOGRAPHY_RAW = {
...
def load_options():
    opts = {
        ...
        "regions": sorted(list(GEOGRAPHY_RAW.keys())),
        "towns": sorted([t for sub in GEOGRAPHY_RAW.values() for t in sub]),
        "brancher": sorted(list(INDUSTRIES_RAW.keys())),
        "underbrancher": sorted([u for sub in INDUSTRIES_RAW.values() for u in sub])
    }
    custom_opts = {k: [] for k in opts.keys()}
    if db_engine:
        try:
...
def lead_popup(idx):
    ...
    with t3:
        ...
        with c1:
            ...
            upd['Advertising'] = st.selectbox(L['f_adv'], opts['advertising'], index=opts['advertising'].index(row['Advertising']) if row['Advertising'] in opts['advertising'] else 0)
...
with st.sidebar:
    ...
    # ADMIN KONTROL (SEPARATE FELTER TIL ALT)
    if st.session_state.user_role == "admin":
        with st.expander(L['sidebar_admin']):
            # BRUGERSTYRING
            st.markdown("##### 👤 Agenter")
            nu, np = st.text_input("Navn"), st.text_input("Pass", type="password")
            if st.button("Opret Agent"):
                db_execute("INSERT INTO users VALUES (:u,:p,'agent')", {"u":nu, "p":np}); st.rerun()
            st.divider()
            # LISTE STYRING
            for label, key in [("Status", "status"), ("Medlem", "membership"), ("Annonce", "advertising"), ("Kilde", "kilde"), ("Sprog", "sprog"), ("Type", "area_types"), ("Region", "regions"), ("By", "towns"), ("Branche", "brancher"), ("Underbranche", "underbrancher")]:
                st.markdown(f"**{label}**")
                v_new = st.text_input(f"Ny {label}:", key=f"add_{key}")
                if st.button(f"Tilføj", key=f"btn_a_{key}"):
                    db_execute("INSERT INTO crm_configs (type, value) VALUES (:t,:v)", {"t":key, "v":v_new}); st.rerun()
                if opts[key]:
                    v_del = st.selectbox(f"Slet fra {label}:", ["Vælg..."] + opts[key], key=f"del_{key}")
                    if v_del != "Vælg..." and st.button(f"Slet", key=f"btn_d_{key}"):
                        db_execute("DELETE FROM crm_configs WHERE type=:t AND value=:v", {"t":key, "v":v_del}); st.rerun()
            st.divider()
    ...
    if st.button(L['btn_create'], type="primary", use_container_width=True):
        ...
        st.session_state.df_leads = pd.concat([st.session_state.df_leads, pd.DataFrame([nr])], ignore_index=True)
        save_db(st.session_state.df_leads); st.rerun()
    st.download_button("📥 Export", st.session_state.df_leads.to_csv(index=False), "master.csv", use_container_width=True)
